...
return Phase_X
        ...
        This function now also returns the primed indices to avoid lookup errors.
            ...
            function precomputeConvolutionTensor(momentum_indices, q_indices, Q_grid_indices)
                ikx, iky = momentum_indices
                iqx, iqy = q_indices
                iQx_val, iQy_val = Q_grid_indices
                ikx_p, iky_p = prime(ikx), prime(iky) # k' indices
                print("hi!")
                ...
                @showprogress "Building convolution tensor..." for kx_idx in 1:dim(ikx), ky_idx in 1:dim(iky), qx_idx in 1:dim(iqx), qy_idx in 1:dim(iqy)
                    k_prime_x_idx = mod1(kx_idx - qx_idx - (iQx_val - 1), dim(ikx))
                    k_prime_y_idx = mod1(ky_idx - qy_idx - (iQy_val - 1), dim(iky))
            ...
            function run_verification_test()
                ...
                # --- 3. Define grids ---
                k_grid_vals_x = range(-π/L, π/L, length=N_k)
                k_grid_vals_y = range(-π/(q_supercell*L), π/(q_supercell*L), length=N_k)
                ...
                Shift, ikx_p, iky_p = precomputeConvolutionTensor(momentum_indices, q_indices, Q_val)
                ...
                for idx_k_x in 1:N_k, idx_k_y in 1:N_k
                ...
                # --- 6. Build Hamiltonian Terms ---
                println("\n--- Building Hamiltonian Terms ---")
                println("Building Direct Term...")
                H_direct = buildDirectTerm(Δ, V_full, S_core_full, S_neg_q_core_full, Phase_D_full, Q_indices, orbital_indices, momentum_indices)
                println("Building Exchange Term...")
                H_exchange = buildExchangeTerm(Δ, V_full, S_core_full, S_neg_q_core_full, Phase_X_full, Shift, ikx_p, iky_p, orbital_indices, momentum_indices)

                println("\n--- Final Hamiltonian Norms ---")
                println("Norm of H_direct:   ", norm(H_direct))
                println("Norm of H_exchange: ", norm(H_exchange))
                println("---------------------------------")
