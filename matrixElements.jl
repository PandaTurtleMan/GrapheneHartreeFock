...
function landauFourierMatrixElement(q_x,q_y,n_1,n_2,l_B)
    ...
    #deltaN = n_1-n_2
    #theta = angle((q_x,q_y),(1.0,0.0))
    phaseFactor = exp(0.5*im*(q_x*q_y*l_B^2))
    return phaseFactor*laguerrePart(n_1,n_2,l_B,q_x,q_y)
end
...
function fourierMatrixElement(k_x, k_y, n1, n2, l1, l2, lambda1, lambda2, spin1, spin2, valley1, valley2, L, p, q, q1, q2)
    ...
    k1 = k_x / K
    k2 = k_y / K
    l_B = sqrt(q/p) * L  # Corrected magnetic length
...
function matrixElement(k_x, k_y, n1, n2, l1, l2, lambda1, lambda2, spin1, spin2, valley1, valley2, L, p, q, all_gvectors)
    total_element = 0.0 + 0.0im
    isempty(all_gvectors) && return total_element

    for (coeff, gx, gy) in all_gvectors
        element = fourierMatrixElement(k_x, k_y, n1, n2, l1, l2, lambda1, lambda2,
                                       spin1, spin2, valley1, valley2, L, p, q, gx, gy)
        total_element += coeff * element
    end
    return total_element
end

function evaluateLandauLevelMatrixElement(n, B, lambda,m)
    hbar = 1.0
    e = 1.0
    v_F = 1.0
